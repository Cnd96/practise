...
readInterface.on('line', function(line) {
    ...
    if(lineCount==1){
       ...
       let totalScore=0
        ...
        libIndex++;
        if(sortedtempLibBooksData.length>150){
...
function calculate(){
    librariesData.sort((a,b) => (b.signUpDate < a.signUpDate) ? 1 : ((a.signUpDate < b.signUpDate) ? -1 : 0)); 
...
function printAnswer(){
    ...
    fs.writeFileSync(fileName+'thun_answer.txt',resultString, function (err) {
        if (err) throw err;
    });
}
