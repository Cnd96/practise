...
function calculate(){
    librariesData.sort((a,b) => (a.booksInLib.length < b.booksInLib.length) ? 1 : ((b.booksInLib.length < a.booksInLib.length) ? -1 : 0)); 
...
function printAnswer(){
    ...
    fs.writeFileSync(fileName+'maxbooks_answer.txt',resultString, function (err) {
        if (err) throw err;
    });
}
